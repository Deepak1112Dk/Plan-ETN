  ...
  if (!isOpen) {
    return (
      <button
        ...
        className="fixed bottom-6 right-6 p-4 bg-gradient-to-r from-orange-500 to-red-600 text-white rounded-full shadow-2xl hover:shadow-orange-300 transition-all transform hover:scale-110 z-50"
      >
    ...
    <div className="fixed bottom-6 right-6 w-96 h-[600px] bg-white rounded-2xl shadow-2xl flex flex-col z-50">
      <div className="bg-gradient-to-r from-orange-500 to-red-600 text-white p-4 rounded-t-2xl flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="font-bold">Tamil Nadu Travel Assistant</h3>
      ...
      <div ref={chatWindowRef} className="flex-1 overflow-y-auto p-4 space-y-4">
        {messages.map((msg, idx) => (
          <div key={idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-[80%] ${msg.role === 'user' ? 'bg-gradient-to-r from-orange-500 to-red-600 text-white' : 'bg-gray-100 text-gray-900'} rounded-2xl p-3`}>
      ...
      <div className="p-4 border-t">
        <div className="flex gap-2">
          ...
          />
          <label htmlFor="chat-image-upload" className="p-2 hover:bg-gray-100 rounded-lg cursor-pointer transition-colors">
            <ImageIcon className="w-5 h-5 text-gray-600" />
          </label>
          <button
            onClick={handleVoiceInput}
            className={`p-2 rounded-lg transition-colors ${isRecording ? 'bg-red-500 text-white' : 'hover:bg-gray-100 text-gray-600'}`}
          >
          ...
          <button
            ...
            className="p-2 bg-gradient-to-r from-orange-500 to-red-600 text-white rounded-lg hover:shadow-lg transition-all disabled:opacity-50 disabled:cursor-not-allowed"
          >
